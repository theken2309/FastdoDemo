		...
		<View className="flex " >
			...
			<View className="flex flex-col gap-4 p-4">
				<View className="flex flex-col gap-4">
					...
					<FlatList
						data={user?.productsFavorite?.filter((product) => product === "timekeeping" || product === "todolist" || product === "cfr" || product === "workflow")}
					...
					<FlatList
						data={user?.products?.filter((product) => product === "timekeeping" || product === "todolist" || product === "cfr" || product === "workflow")}
						keyExtractor={(item, index) => `${index}-${item}`}
						numColumns={4}
						renderItem={({ item }) => (
						<View className = "flex items-center px-6" > 
